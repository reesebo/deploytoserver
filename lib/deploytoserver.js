  ...
  deploy(action){
		...
		this.opencfg((cfg,cfg_status,msg)=>{
			if(cfg_status){
				...
				for(var key in dependencies){
					host.commands.push('npm install ' + key +' --save');
				}
	...
	findRouter(path){
		...
		for(var item in subs){
			var nextPath = path + "\\" + subs[item];
			if(fs.lstatSync(nextPath).isDirectory()){
				this.findRouter(nextPath);
			}
			else{
				if(this.isSelectedFile(nextPath,['.js'])){
					var fileContent = fs.readFileSync(nextPath);
					if(fileContent.indexOf("global.web.initRouter(this)") >= 0){
						this.routerFiles.push(nextPath);
					}
				}
			}
		}
		...
		this.opencfg((cfg,cfg_status,msg)=>{
			...
			if(cfg_status){
				if("webpath" in cfg){
					...
					this.findRouter(webpath);
					var allRouters = {};
					this.routerFiles.forEach((e)=>{
						...
						allRouters[routerRelativeFile] ='.' + routerRelativeFile + ".js";

					});
					var routerJsonPath = _this.joinPath(webpath,"router.json");
					fs.writeFileSync(routerJsonPath,JSON.stringify(allRouters));
					_this.printMessage('router.json generated.','panel','info');
	...
	registerglobalmodel(action){
		this.opencfg((cfg,cfg_status,msg)=>{
			...
			if(cfg_status){
				if("webpath" in cfg){
					var webpath = _this.joinPath(_this.rootDirPath,cfg.webpath);
					this.findModel(webpath);
					var allModels = {};
					this.modelFiles.forEach((e)=>{
						var serverPath = e.replace(this.rootDirPath,"").replace(/\\/g,'/');
						...
						allModels[modelRelativeFile] ='.' + modelRelativeFile + ".js";

					});
					var modelJsonPath = _this.joinPath(webpath,"globalmodel.json");
					fs.writeFileSync(modelJsonPath,JSON.stringify(allModels));
					_this.printMessage('globalmodel.json generated.','panel','info');
